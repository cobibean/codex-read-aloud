...
import { configPath, readConfig, readWrapper } from "./lib/read-aloud.mjs";
...
check("Codex config", "~/.codex/config.toml", existsSync(resolve(homedir(), ".codex", "config.toml")));
check("Codex notify wrapper", wrapper.notifyCommand?.join(" ") || "not installed", Array.isArray(wrapper.notifyCommand));
...
if (checks.some((item) => !item.ok && item.name !== "OpenAI key" && item.name !== "Codex notify wrapper" && item.name !== "Claude Code CLI" && item.name !== "Codex CLI")) {
  process.exit(1);
}
